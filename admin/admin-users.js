...
export async function showUsuariosView() {
    ...
    if (currentUserLevel === 'superAdmin') {
        ...
        usersQuery = query(collection(db, "usuarios"), where("acessoInstituicoes", "array-contains-any", currentUserInstitutions));
    }
                ...
                ${users.map(user => `
                    <tr>
                        ...
                        <td class="admin-actions">
                            <button class="admin-button-edit" data-id="${user.id}">Editar</button>
    ...
    document.getElementById('add-new-user').addEventListener('click', () => openUserModal(null));
...
async function openUserModal(userId) {
    ...
    if (currentUserLevel === 'superAdmin') {
        nivelOptions = `
            ...
            <option value="superAdmin" ${user.nivel === 'superAdmin' ? 'selected' : ''}>Super Admin</option>
        `;
    } else {
        nivelOptions = `<option value="operador" selected>Operador</option>`;
    }
    
    // ==============================================================
    // === INÍCIO DA MODIFICAÇÃO (HTML Lógica de Senha) ===
    // ==============================================================
    ...
    // NOVO: Regras de senha para o tooltip
    const passwordRules = "Deve ter mín. 6 caracteres, 1 maiúscula, 1 minúscula, 1 número e 1 caractere especial (ex: @, $, !, %, *, ?, &).";
    ...
    modalContent.innerHTML = `
        ...
        <form id="user-form">
            <div class="form-group">
                ...
                <input type="text" id="nome" value="${user.nome || ''}" required>
            </div>
            <div class="form-group">
                <label for="email">Email</label>
                <input type="email" id="email" value="${user.email || ''}" ${userId ? 'disabled' : ''} required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                    <input type="checkbox" id="alarmesAtivos" style="width: auto;" ${user.alarmesAtivos !== false ? 'checked' : ''}>
    ...
    modalOverlay.querySelector('#user-form').addEventListener('submit', (e) => {
        e.preventDefault();
        saveUser(userId, selectedInst, selectedUnit, selectedSetor, selectedDispositivo); 
    });
}
    ...
    instituicoesPermitidas.forEach(inst => {
        ...
        li.addEventListener('click', (e) => {
            if (e.target.tagName !== 'INPUT') { 
                instList.querySelectorAll('li').forEach(item => item.classList.remove('active'));
...
function renderAccessUnidades(instId) {
    ...
    unidadesPermitidas
        ...
        .forEach(unit => {
            ...
            const li = createAccessListItem(unit.id, unit.nome, isChecked, 'unit', { instId });
            li.addEventListener('click', (e) => {
                if (e.target.tagName !== 'INPUT') {
...
function renderAccessSetores(unitId) {
    ...
    setoresPermitidos
        ...
        .forEach(setor => {
            ...
            const li = createAccessListItem(setor.id, setor.nome, isChecked, 'setor', { unitId, instId: unit.instituicaoId });
            li.addEventListener('click', (e) => {
                if (e.target.tagName !== 'INPUT') {
                    setorList.querySelectorAll('li').forEach(item => item.classList.remove('active'));
                    li.classList.add('active');
...
function renderAccessDispositivos(setorId) {
    ...
    const dispositivos = dispositivosPermitidos.filter(d => d.setorID === setorId);

    if (dispositivos.length === 0) {
        dispositivoList.innerHTML = '<li><em>Nenhum dispositivo encontrado</em></li>';
        return;
    }
...
function cascadeCheck(id, type, parentIds, check = true) {
    ...
    if (type === 'unit') {
        hierarchyCache.setores
            ...
            .forEach(setor => cascadeCheck(setor.id, 'setor', { unitId: id, instId: parentIds.instId }, check));
    }

    if (type === 'setor') {
        hierarchyCache.dispositivos
            .filter(dispositivo => dispositivo.setorID === id)
            .forEach(dispositivo => cascadeCheck(dispositivo.id, 'dispositivo', { setorID: id, unitId: parentIds.unitId, instId: parentIds.instId }, check));
    }
}
...
function bubbleUncheck(id, type, parentIds) {
    if (type === 'dispositivo') {
        ...
        bubbleUncheck(parentIds.setorId, 'setor', { unitId: parentIds.unitId, instId: parentIds.instId });
    }
    if (type === 'setor') {
...
function refreshAccessTreeUI() {
    document.querySelectorAll('#access-inst-list input[type="checkbox"]').forEach(chk => {
        chk.checked = selectedInst.has(chk.dataset.id);
    });
    document.querySelectorAll('#access-unit-list input[type="checkbox"]').forEach(chk => {
        chk.checked = selectedUnit.has(chk.dataset.id);
    });
    document.querySelectorAll('#access-setor-list input[type="checkbox"]').forEach(chk => {
        chk.checked = selectedSetor.has(chk.dataset.id);
    });
    document.querySelectorAll('#access-dispositivo-list input[type="checkbox"]').forEach(chk => {
...
function getPermittedHierarchy() {
    ...
    return { instituicoesPermitidas, unidadesPermitidas, setoresPermitidos, dispositivosPermitidos };
}
...
function validatePassword(password) {
    ...
    if (!/(?=.*[@$!%*?&])/.test(password)) {
        errors.push("Ter pelo menos 1 caractere especial (ex: @, $, !, %, *, ?, &).");
    }

    if (errors.length > 0) {
        return {
            isValid: false,
            message: "A senha não atende aos seguintes requisitos:<br><ul><li>" + errors.join("</li><li>") + "</li></ul>"
        };
    }
    return { isValid: true };
}
    ...
    try {
        ...
        if (isNew) {
            ...
            if (senha !== senhaConfirm) {
                showNotification("As senhas não conferem. Por favor, digite novamente.", 'error');
                return;
            }
        ...
        if (error.code === "auth/email-already-in-use") {
             ...
             showNotification("A senha fornecida é muito fraca (erro do Firebase).", 'error');
        } else {
...
async function deleteUserFirestore(userId, email) {
    ...
    try {
        ...
        showNotification(`Documento do usuário ${email} excluído do Firestore. O login (Auth) AINDA EXISTE.`, 'info');
        showUsuariosView();
    } catch (error) {
