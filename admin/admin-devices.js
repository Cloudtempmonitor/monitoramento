// ==========================================================================
// ADMIN-DEVICES.JS — Gerenciamento de Dispositivos 
// ==========================================================================
...
// ============================================================
// Inicialização do módulo (injeção de dependências)
// ============================================================
export function initDevicesModule(dependencies) {
  ({ db, currentUserLevel, currentUserInstitutions, hierarchyCache } =
    dependencies);
}
...
export async function showDispositivosView() {
  ...
  const devicesQuery =
    currentUserLevel === "superAdmin"
      ...
      ? query(
          ...
          where("instituicaoID", "in", currentUserInstitutions)
        )
      : null;
  ...
  contentArea.innerHTML = `
    ...
    <table id="admin-table">
      <thead>
        <tr><th>Nome</th><th>MAC (ID)</th><th>Setor</th><th>Instituição</th><th>Status</th><th>Ações</th></tr>
      </thead>
      <tbody>
        ${devices
          .map(
          ...
          <tr>
            ...
            <td>${
              device.sondaAtiva === false
                ? '<span style="color: #e74c3c;">Sonda Inativa</span>'
                : '<span style="color: #2ecc71;">Sonda Ativa</span>'
            }</td>
            <td class="admin-actions">
              <button class="admin-button-edit" data-id="${device.id}">Editar</button>
              ${
                currentUserLevel === "superAdmin"
                  ? `<button class="admin-button-delete" data-id="${device.id}">Excluir</button>`
                  : ""
              }
            </td>
          </tr>`
  ...
  document
    .getElementById("add-new-device")
    .addEventListener("click", () => openDeviceModal(null));

  document
    .querySelectorAll(".admin-button-edit")
    .forEach((btn) =>
      btn.addEventListener("click", (e) => openDeviceModal(e.target.dataset.id))
    );

  // Excluir dispositivo 
  if (currentUserLevel === "superAdmin") {
    document.querySelectorAll(".admin-button-delete").forEach((btn) =>
      btn.addEventListener("click", async (e) => {
        const id = e.target.dataset.id;

        const confirmed = await showConfirmation(
          `Tem certeza que deseja excluir o dispositivo ${id}?`,
          "Excluir Dispositivo"
        );
...
// ============================================================
// Formulário de edição / novo dispositivo
// ============================================================
async function openDeviceModal(deviceId) {
  ...
  modalContent.innerHTML = `
    ...
    <form id="device-form">
      <div class="form-group">
        <label for="mac">MAC Address (ID)</label>
        <input type="text" id="mac" value="${deviceId || ""}" ${
    deviceId ? "disabled" : ""
  }>
        ${
          !deviceId
            ? "<small>Após criado, o MAC não pode ser alterado.</small>"
            : ""
        }
      </div>
      <div class="form-group">
        <label for="nomeDispositivo">Nome do Dispositivo</label>
        <input type="text" id="nomeDispositivo" value="${
      ...
      <div class="form-group">
        <label>Sonda Externa (Sensor de Contato)</label>
        <div style="display: flex; align-items: center; gap: 10px;">
          <input type="checkbox" id="sondaAtiva" style="width: auto;" ${
            isSondaAtiva ? "checked" : ""
          }>
          <label for="sondaAtiva" style="margin-bottom: 0; font-weight: normal; cursor: pointer;">Sonda Ativa (Monitora temp. do sensor de contato)</label>
        </div>
        <small>Se desmarcado, o dashboard focará apenas na Temperatura e Umidade Ambiente.</small>
      </div>

      <hr><h4>Limites de Alarme</h4>
      <div class="form-group">
        <div class="alarm-grid-container">
          <div class="alarm-grid">
            <span class="alarm-grid-header"><span>Tipo</span></span>
            <span class="alarm-grid-header">Mínimo</span>
            <span class="alarm-grid-header">Máximo</span>

            <span class="alarm-grid-label">Sonda (°C)</span>
            <input type="number" step="0.1" id="alarmeMin" value="${
              device.alarmeMin?.sonda || ""
            }" placeholder="Ex: 2.0">
            <input type="number" step="0.1" id="alarmeMax" value="${
              device.alarmeMax?.sonda || ""
            }" placeholder="Ex: 8.0">

            <span class="alarm-grid-label">Ambiente (°C)</span>
            <input type="number" step="0.1" id="alarmeMinAmb" value="${
              device.alarmeMin?.temperaturaAmbiente || ""
            }" placeholder="Ex: 15.0">
            <input type="number" step="0.1" id="alarmeMaxAmb" value="${
              device.alarmeMax?.temperaturaAmbiente || ""
            }" placeholder="Ex: 30.0">

            <span class="alarm-grid-label">Umidade (%)</span>
            <input type="number" step="1" id="alarmeMinUmid" value="${
              device.alarmeMin?.umidade || ""
            }" placeholder="Ex: 40">
            <input type="number" step="1" id="alarmeMaxUmid" value="${
              device.alarmeMax?.umidade || ""
            }" placeholder="Ex: 60">
          </div>
        </div>
      </div>
      ...
      <div class="form-group">
        <label>Ativar Alarmes por Sensor</label>
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <div style="display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" id="alarmeSondaAtivo" style="width: auto;" ${
              isAlarmeSondaAtivo ? "checked" : ""
            } ${isSondaAtiva ? "" : "disabled"}>
            <label for="alarmeSondaAtivo" style="margin-bottom: 0; font-weight: normal; cursor: pointer;">Alarme para Sonda (Ativo apenas se Sonda estiver Ativa)</label>
          </div>

          <div style="display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" id="alarmeTempAmbienteAtivo" style="width: auto;" ${
              isAlarmeTempAmbienteAtivo ? "checked" : ""
            }>
            <label for="alarmeTempAmbienteAtivo" style="margin-bottom: 0; font-weight: normal; cursor: pointer;">Alarme para Temperatura Ambiente</label>
          </div>

          <div style="display: flex; align-items: center; gap: 10px;">
            <input type="checkbox" id="alarmeUmidadeAtivo" style="width: auto;" ${
              isAlarmeUmidadeAtivo ? "checked" : ""
            }>
            <label for="alarmeUmidadeAtivo" style="margin-bottom: 0; font-weight: normal; cursor: pointer;">Alarme para Umidade</label>
          </div>
        </div>
      </div>
  ...
  (currentUserLevel === "superAdmin"
    ...
    : hierarchyCache.instituicoes.filter((inst) =>
        currentUserInstitutions.includes(inst.id)
      )
  ).forEach((inst) => {
    instSelect.innerHTML += `<option value="${inst.id}">${
      inst.nome || inst.id
    }</option>`;
  });
  ...
  unitSelect.addEventListener("change", () => {
    ...
    if (id)
      hierarchyCache.setores
        .filter((s) => s.unidadeId === id)
        .forEach(
          (s) =>
            (setorSelect.innerHTML += `<option value="${s.id}">${
              s.nome || s.id
            }</option>`)
        );
  });
  ...
  if (deviceId && device.instituicaoID) {
    ...
    setTimeout(() => {
      if (device.unidadeID) {
        unitSelect.value = device.unidadeID;
        unitSelect.dispatchEvent(new Event("change"));
      }
    }, 100);
    setTimeout(() => {
      if (device.setorID) setorSelect.value = device.setorID;
    }, 200);
  }
  ...
  modalOverlay
    ...
    .addEventListener("click", () => document.body.removeChild(modalOverlay));

  modalOverlay.querySelector("#device-form").addEventListener("submit", (e) => {
    e.preventDefault();
    saveDevice(deviceId);
  });
}
...
async function saveDevice(deviceId) {
  ...
  if (!mac) return showNotification("O MAC Address é obrigatório.", "error");
  ...
  const deviceData = {
    ...
    alarmeTempAmbienteAtivo: document.getElementById(
      "alarmeTempAmbienteAtivo"
    ).checked,
  ...
  try {
    ...
    if (isNew) {
      ...
      showNotification(
        "Dispositivo atualizado com sucesso! O dispositivo será reiniciado.",
        "success"
      );
    }
